...
router.post("/invoice", async (req, res) => {
    ...
    if (!data) {
        return res.status(400).json({ resultCode: "400", mestbinvoicesage: "Data is empty!" });
    }
...
router.get("/invoice/:id", async (req, res) => {
    ...
    try {
        db.query(selectInvoiceIds, (err, result) => {
            if (err) {
                return res.status(500).json({ message: "Database error", error: err.message });
            }

            if (result.length === 0) {
                return res.status(404).json({ message: "No invoice IDs found" });
            }
